...
const map = {
  ...
  init(rowsCount, colsCount) {
    ...
    for (let row = 0; row < rowsCount; row++) {
      ...
      for (let col = 0; col < colsCount; col++) {
        ...
        td.classList.add(`x${col.toString()}_y${row.toString()}`);
  ...
  render(snakePointsArray, foodPoint) {
    ...
    document.querySelector("#counter").innerHTML = snake.bodyLenght;
    document.querySelector("#max-count").innerHTML = `/ ${settings.winFoodCount}`;
  },
};
...
const snake = {
  ...
  /**
   * Проверяет содержит ли змейка переданную точку (без учета головы).
  ...
  makeStep() {
    ...
    this.body[0] = this.getNextStepHeadPoint();
...
const game = {
  ...
  getStartSnakeBody() {
    return [
      ...
      // {
      //   x: Math.floor(this.config.getColsCount() / 2),
      //   y: Math.floor(this.config.getRowsCount() / 2) + 1,
      // },
      // {
      //   x: Math.floor(this.config.getColsCount() / 2),
      //   y: Math.floor(this.config.getRowsCount() / 2) + 2,
      // },
      // {
      //   x: Math.floor(this.config.getColsCount() / 2),
      //   y: Math.floor(this.config.getRowsCount() / 2) + 3,
      // },
    ];
  },
   ...
   */
  isGameWon() {
    return this.snake.bodyLenght === 5;
  },
  ...
  canMakeStep() {
    for (let i = 0; i < this.snake.body.length; i++)
      if (this.snake.isOnPoint(this.snake.body[0]))
        return false;
